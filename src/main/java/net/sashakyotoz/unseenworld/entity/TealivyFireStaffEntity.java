...
public class TealivyFireStaffEntity extends AbstractArrow implements ItemSupplier{
    ...
    public void onHitBlock(BlockHitResult blockHitResult) {
        ...
        this.level().addParticle(UnseenWorldModParticleTypes.BLUE_VOID_PARTICLE.get(), blockHitResult.getBlockPos().getX(), blockHitResult.getBlockPos().getY(), blockHitResult.getBlockPos().getZ(), 0, 1, 0);
    }

    @Override
    public void tick() {
        super.tick();
        FireLikeStaffProjectileFlyingTick.onTickParticles(UnseenWorldModParticleTypes.BLUE_VOID_PARTICLE.get(),this.level(), this.getX(), this.getY(), this.getZ(),0.5f,0,1f);
        if (this.inGround) {
    ...
    private TealivyFireStaffEntity shootSmallShard(BlockPos targetPos) {
        TealivyFireStaffEntity staffEntity = new TealivyFireStaffEntity(UnseenWorldModEntities.TEALIVY_FIRE_STAFF.get(), (LivingEntity) this.getOwner(), this.level());
        ...
        this.level().playSound(null, this.getX(), this.getY(), this.getZ(), UnseenWorldModSounds.ITEM_STAFF_SHOT, SoundSource.PLAYERS, 1, 1f / (RandomSource.create().nextFloat() * 0.5f + 1));
        return staffEntity;
    }

    @Override
    public ItemStack getItem() {
        return UnseenWorldModItems.TANZANITE_SHARD.get().getDefaultInstance();
    }
}
