...
public class EntityInteractWithWitherKnightBlockProcedure {
	public static void execute(Level level, BlockPos pos) {
		if (!(level.getDifficulty() == Difficulty.PEACEFUL)) {
			...
			EventManager.waveFlaming(UnseenWorldModParticleTypes.GOLDEN.get(),level,pos);
			UnseenWorldMod.queueServerWork(20, () -> {
				if (level instanceof ServerLevel serverLevel)
					UnseenWorldModEntities.THE_WITHER_KNIGHT.get().spawn(serverLevel, pos.above(), MobSpawnType.MOB_SUMMONED);
			});
		}
	}
}