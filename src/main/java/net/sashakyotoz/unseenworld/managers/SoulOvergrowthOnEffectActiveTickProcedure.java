	...
	public static void execute(LevelAccessor level, double x, double y, double z, Entity entity) {
		...
		for (int i = 0; i < 5; i++) {
			...
			for (int j = 0; j < 3; j++) {
				sz = -3;
				for (int k = 0; k < 5; k++) {
					if (((level.getBlockState(BlockPos.containing(x + sx, y + sy, z + sz))).getBlock() == Blocks.GRASS_BLOCK || (level.getBlockState(BlockPos.containing(x + sx, y + sy, z + sz))).getBlock() == UnseenWorldModBlocks.DARK_GRASS_BLOCK.get())
							&& (level.getBlockState(BlockPos.containing(x + sx, y + sy + 1, z + sz))).getBlock() == Blocks.AIR) {
