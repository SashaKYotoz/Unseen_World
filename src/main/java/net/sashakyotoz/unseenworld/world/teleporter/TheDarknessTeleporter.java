...
public class TheDarknessTeleporter implements ITeleporter {
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("the_darkness_portal", Vec3i::compareTo, 300);
	public static Holder<PoiType> poi = null;

	@SubscribeEvent
	public static void registerPointOfInterest(RegisterEvent event) {
		event.register(ForgeRegistries.Keys.POI_TYPES, registerHelper -> {
			PoiType poiType = new PoiType(ImmutableSet.copyOf(UnseenWorldModBlocks.THE_DARKNESS_PORTAL.get().getStateDefinition().getPossibleStates()), 0, 1);
			registerHelper.register("the_darkness_portal", poiType);
	...
	public TheDarknessTeleporter(ServerLevel worldServer, BlockPos entityEnterPos) {
		this.level = worldServer;
		this.entityEnterPos = entityEnterPos;
	}

	public Optional<BlockUtil.FoundRectangle> findPortalAround(BlockPos pos, boolean p_192987_, WorldBorder border) {
		PoiManager poimanager = this.level.getPoiManager();
		int i = p_192987_ ? 16 : 128;
		poimanager.ensureLoadedAndValid(this.level, pos, i);
		Optional<PoiRecord> optional = poimanager.getInSquare((p_230634_) -> p_230634_.is(poi.unwrapKey().get()), pos, i, PoiManager.Occupancy.ANY).filter((p_192981_) -> border.isWithinBounds(p_192981_.getPos())).sorted(Comparator.<PoiRecord>comparingDouble((p_192984_) -> p_192984_.getPos().distSqr(pos)).thenComparingInt((p_192992_) -> p_192992_.getPos().getY())).filter((p_192990_) -> this.level.getBlockState(p_192990_.getPos()).hasProperty(BlockStateProperties.HORIZONTAL_AXIS)).findFirst();
			...
			for (int i3 = -1; i3 < 2; ++i3) {
				for (int j3 = 0; j3 < 2; ++j3) {
					for (int k3 = -1; k3 < 3; ++k3) {
						BlockState blockstate1 = k3 < 0 ? UnseenWorldModBlocks.COLD_DARK_BRICKS.get().defaultBlockState() : Blocks.AIR.defaultBlockState();
		...
		for (int l1 = -1; l1 < 3; ++l1) {
			for (int j2 = -1; j2 < 4; ++j2) {
				if (l1 == -1 || l1 == 2 || j2 == -1 || j2 == 3) {
					...
					this.level.setBlock(blockpos$mutableblockpos, UnseenWorldModBlocks.COLD_DARK_BRICKS.get().defaultBlockState(), 3);
				}
			}
		}
		BlockState blockstate = UnseenWorldModBlocks.THE_DARKNESS_PORTAL.get().defaultBlockState().setValue(NetherPortalBlock.AXIS, axis);
	...
	private boolean canHostFrame(BlockPos blockPos, BlockPos.MutableBlockPos pos, Direction direction1, int p_77665_) {
		Direction direction = direction1.getClockWise();
		for (int i = -1; i < 3; ++i) {
			for (int j = -1; j < 4; ++j) {
				pos.setWithOffset(blockPos, direction1.getStepX() * i + direction.getStepX() * p_77665_, j, direction1.getStepZ() * i + direction.getStepZ() * p_77665_);
				if (j < 0 && !this.level.getBlockState(pos).isSolid()) {
					return false;
				}
				if (j >= 0 && !this.canPortalReplaceBlock(pos)) {
					return false;
				}
			}
		}
	...
	private PortalInfo getPortalInfo(Entity entity, ServerLevel server) {
		...
		return this.getExitPortal(entity, blockpos1, worldborder).map(repositioner -> {
			...
			if (blockstate.hasProperty(BlockStateProperties.HORIZONTAL_AXIS)) {
				...
				vector3d = TheDarknessPortalShape.getRelativePosition(teleportationrepositioner$result, direction$axis, entity.position(), entity.getDimensions(entity.getPose()));
			} else {
				direction$axis = Direction.Axis.X;
				vector3d = new Vec3(0.5, 0, 0);
			}
			return TheDarknessPortalShape.createPortalInfo(server, repositioner, direction$axis, vector3d, entity, entity.getDeltaMovement(), entity.getYRot(), entity.getXRot());
