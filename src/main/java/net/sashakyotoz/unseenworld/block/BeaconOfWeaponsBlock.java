...
public class BeaconOfWeaponsBlock extends BaseEntityBlock {
    ...
    public static boolean isValidRune(Level level, BlockPos pos, BlockPos pos1) {
        return level.getBlockState(pos.offset(pos1)).is(UnseenWorldModBlocks.BEACON_RUNE.get()) && level.getBlockState(pos.offset(pos1.getX() / 2, pos1.getY(), pos1.getZ() / 2)).is(BlockTags.REPLACEABLE);
    }
    ...
    public void animateTick(BlockState state, Level level, BlockPos pos, RandomSource source) {
        ...
        for (BlockPos blockpos : RUNE_OFFSETS) {
            if (source.nextInt(12) == 0 && isValidRune(level, pos, blockpos))
                level.addParticle(level.getBiome(pos).is(Tags.Biomes.IS_HOT) ? UnseenWorldModParticleTypes.REDNESS.get() : ModParticleTypes.WISP_LIKE_PARTICLE.get(),
    ...
    public static boolean isRuneNear(Level level, BlockPos pos) {
        ...
        for (int x = -3; x < 3; x++) {
            for (int z = -3; z < 3; z++) {
                ...
                if (tmpState.is(UnseenWorldModBlocks.BEACON_RUNE.get()) && tmp != 4) {
                    tmp++;
                    x = -3;
                    z = -3;
                }
