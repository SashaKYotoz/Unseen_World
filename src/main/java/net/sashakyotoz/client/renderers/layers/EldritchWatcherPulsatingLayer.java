...
public class EldritchWatcherPulsatingLayer<T extends EldritchWatcherEntity, M extends SinglePartEntityModel<T>> extends FeatureRenderer<T, M> {
    ...
    public void render(MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, T entity, float limbAngle, float limbDistance, float tickDelta, float animationProgress, float headYaw, float headPitch) {
        ...
        this.getContextModel().render(matrices, vertexConsumer, light, OverlayTexture.DEFAULT_UV, 1.0F, 1.0F, 1.0F, (float) Oscillator.getOscillatingValue(Math.round(WorldClientTickEventHandler.halfTicks.get(0) * 2)));
    }
}