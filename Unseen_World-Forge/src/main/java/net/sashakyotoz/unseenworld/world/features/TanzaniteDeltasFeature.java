...
public class TanzaniteDeltasFeature extends Feature<NoneFeatureConfiguration> {
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        ...
        if (!generateDimensions.contains(worldgenlevel.getLevel().dimension()) || !worldgenlevel.isEmptyBlock(blockpos))
            return false;
        worldgenlevel.setBlock(blockpos, UnseenWorldBlocks.TANZASHROOM_LIGHT.get().defaultBlockState(), 2);
        for (int i = 0; i < 1500; ++i) {
            ...
            if (worldgenlevel.getBlockState(blockpos1).isAir()) {
                ...
                for (Direction direction : Direction.values()) {
                    if (worldgenlevel.getBlockState(blockpos1.relative(direction)).is(UnseenWorldBlocks.TANZASHROOM_LIGHT.get()))
                        ++j;
                    if (j > 1)
                        break;
                }
                if (j == 1) {
                    worldgenlevel.setBlock(blockpos1, UnseenWorldBlocks.TANZASHROOM_LIGHT.get().defaultBlockState(), 2);
                }
            }
        }
        return true;
