        ...
        if (stack.isIn(ModTags.Items.CAN_BE_CHARGED_BY_GRIPCRYSTALS) && GripcrystalManaData.getMana((IEntityDataSaver) player) > 0) {
            ...
            if (stack.getItem() instanceof ChimericRockbreakerHammerItem
                ...
                for (int i1 = 0; i1 < 10; i1++) {
                    BlockPos pos = player.getWorld().raycast(new RaycastContext(player.getEyePos(), player.getEyePos().add(player.getRotationVec(1f).multiply(scaling)), RaycastContext.ShapeType.OUTLINE, RaycastContext.FluidHandling.NONE, player)).getBlockPos();
                    if (!player.getWorld().getBlockState(pos).isOpaque())
                        scaling = scaling + 1;
                    else
